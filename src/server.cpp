...
void Server::run() {
        ...
        for (short j = 0; j < 1024; ++j) {
            ...
            for (short x = 0; x < 64; ++x)
                for (short y = 0; y < 64; ++y)
                    if (field.blocking((i - 512) * 64 + x, (j - 512) * 64 + y))
                        nb++;
            areas[i + j * 1024].first = 0;
            areas[i + j * 1024].second = static_cast<char>((1024 - nb) / 30);
        }
    ...
    std::thread ms{&InformationServer::MonsterCreation, std::ref(monster_thread_run),
                   ...
                   std::ref(players), std::ref(players_guard), std::ref(areas)};

    boost::asio::io_context io_is;
    boost::asio::io_context io_as;
    InformationServer is{io_is, players, std::ref(players_guard), monsters,
                         std::ref(monsters_guard), port, messages, field};


    ActionServer as{io_as, players, std::ref(players_guard), monsters,
                    std::ref(monsters_guard), port, messages, field, areas};
